...
export default function App() {
  ...
  const validate = (): boolean => {
    ...
    loads.forEach((ld) => {
      if (ld.type === 'point') {
        ...
        if (pos < 0 || pos > cfg.length) {
          newErrors[`load-${ld.id}-pos`] = `Must be 0 to ${toDisplay(cfg.length, 'length', unitSystem).toFixed(1)}`;
          isValid = false;
        }
      } else {
        // UDL or Triangular
        ...
        // Start Check
        if (start < 0 || start > cfg.length) {
          newErrors[`load-${ld.id}-start`] = `0 to ${toDisplay(cfg.length, 'length', unitSystem).toFixed(1)}`;
          isValid = false;
        }
        // End Check
        if (end < 0 || end > cfg.length) {
           newErrors[`load-${ld.id}-end`] = `0 to ${toDisplay(cfg.length, 'length', unitSystem).toFixed(1)}`;
           isValid = false;
        }
        // Logic Check
        if (start >= end) {
           newErrors[`load-${ld.id}-range`] = "Start must be < End";
           isValid = false;
        }
      }
    });
  ...
  const handleRunAnalysis = () => {
    if (!validate()) {
      setRes({ error: "Configuration errors found. Please check inputs marked in red." });
      return;
    }
  ...
  const updateLoad = (id: string, updates: Partial<Load>) => {
    ...
    // Clear errors for this load when user types
    const newErrors = { ...errors };
    Object.keys(newErrors).forEach(key => {
      if (key.startsWith(`load-${id}`)) delete newErrors[key];
    });
    setErrors(newErrors);
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 pb-20 font-sans flex flex-col">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 font-medium">For learning</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
      ...
      <main className="max-w-7xl mx-auto px-4 py-8 grid grid-cols-1 lg:grid-cols-12 gap-8 flex-grow">
                        ...
                        <div className="space-y-3">
                          ...
                          {ld.type === 'point' ? (
                            <div className="grid grid-cols-2 gap-2">
                               <div>
                                 <Label className={`text-xs ${errors[`load-${ld.id}-pos`] ? 'text-red-500' : ''}`}>Pos ({u('length')})</Label>
                                 <UnitInput 
                                    value={ld.position} 
                                    onChange={(v) => updateLoad(ld.id, {position: v})} 
                                    unitSystem={unitSystem} 
                                    unitType="length" 
                                    className={errors[`load-${ld.id}-pos`] ? "border-red-500 bg-red-50" : ""}
                                 />
                                 {errors[`load-${ld.id}-pos`] && <p className="text-[10px] text-red-500 mt-0.5">{errors[`load-${ld.id}-pos`]}</p>}
                               </div>
                             ...
                             <div className="grid grid-cols-3 gap-2">
                               <div>
                                 <Label className={`text-xs ${errors[`load-${ld.id}-start`] ? 'text-red-500' : ''}`}>Start ({u('length')})</Label>
                                 <UnitInput 
                                    value={ld.startPosition} 
                                    onChange={(v) => updateLoad(ld.id, {startPosition: v})} 
                                    unitSystem={unitSystem} 
                                    unitType="length" 
                                    className={errors[`load-${ld.id}-start`] || errors[`load-${ld.id}-range`] ? "border-red-500 bg-red-50" : ""}
                                 />
                                 {errors[`load-${ld.id}-start`] && <p className="text-[10px] text-red-500 mt-0.5">{errors[`load-${ld.id}-start`]}</p>}
                               </div>
                               <div>
                                 <Label className={`text-xs ${errors[`load-${ld.id}-end`] ? 'text-red-500' : ''}`}>End ({u('length')})</Label>
                                 <UnitInput 
                                    value={ld.endPosition} 
                                    onChange={(v) => updateLoad(ld.id, {endPosition: v})} 
                                    unitSystem={unitSystem} 
                                    unitType="length" 
                                    className={errors[`load-${ld.id}-end`] || errors[`load-${ld.id}-range`] ? "border-red-500 bg-red-50" : ""}
                                 />
                                 {errors[`load-${ld.id}-end`] && <p className="text-[10px] text-red-500 mt-0.5">{errors[`load-${ld.id}-end`]}</p>}
                               </div>
                               ...
                               {errors[`load-${ld.id}-range`] && <p className="col-span-3 text-[10px] text-red-500">{errors[`load-${ld.id}-range`]}</p>}
                            </div>
                          ) : (
                             <div className="grid grid-cols-2 gap-2">
                               <div>
                               ...
                               {errors[`load-${ld.id}-range`] && <p className="col-span-2 text-[10px] text-red-500">{errors[`load-${ld.id}-range`]}</p>}
                             </div>
                          )}
                        </div>
                      </div>
      ...
      <footer className="w-full text-center py-4 text-xs text-slate-400 bg-slate-50">
        ⚠️ Not to be used for professional work
      </footer>
    </div>
  );
}